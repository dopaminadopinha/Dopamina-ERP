...
begin
 ...
 select jsonb_build_object(
  ...
  'zig_consumption_by_day',(
    select coalesce(jsonb_agg(jsonb_build_object(
      'employee_id', x.employee_id, 'operational_date', x.operational_date,
      'sale_cents', x.sale_cents, 'cost_cents', x.cost_cents
    )), '[]'::jsonb)
    from (
      select c.employee_id, c.operational_date,
        sum(i.net_amount_cents) as sale_cents,
        sum(round(i.quantity * coalesce(h.unit_cost, 0) * 100)) as cost_cents
      from public.employee_zig_consumption c
      join public.zig_transaction_items i on i.transaction_id = c.zig_transaction_row_id and not i.is_refunded
      left join lateral (
        select uc.unit_cost from public.item_cost_history uc
        where uc.item_id = i.item_id and uc.business_id = p_business_id and uc.effective_from <= c.operational_date
        order by uc.effective_from desc limit 1
      ) h on true
      where c.business_id = p_business_id and c.operational_date between p_period_start and p_period_end
      group by c.employee_id, c.operational_date
    ) x
  )
 ) into v_result;
 return v_result;
...
create function public.get_employee_day_consumption(
  p_business_id bigint, p_employee_id bigint, p_date date
) returns jsonb language plpgsql stable security invoker set search_path = '' as $$
declare v_result jsonb;
begin
  if (select auth.uid()) is null or not (select private.is_business_member(p_business_id)) then raise exception 'Acesso negado ao negócio'; end if;
  ...
  select jsonb_build_object(
    'sale_cents', coalesce(sum(i.net_amount_cents), 0),
    'cost_cents', coalesce(sum(round(i.quantity * coalesce(h.unit_cost, 0) * 100)), 0),
    'items', coalesce(jsonb_agg(jsonb_build_object(
      'product_name', i.product_name,
      'quantity', i.quantity,
      'unit_cost_cents', round(coalesce(h.unit_cost, 0) * 100),
      'line_cost_cents', round(i.quantity * coalesce(h.unit_cost, 0) * 100),
      'line_sale_cents', i.net_amount_cents,
      'has_cost', h.unit_cost is not null
    ) order by i.id), '[]'::jsonb)
  ) into v_result
  from public.employee_zig_consumption c
  join public.zig_transaction_items i on i.transaction_id = c.zig_transaction_row_id and not i.is_refunded
  left join lateral (
    select uc.unit_cost from public.item_cost_history uc
    where uc.item_id = i.item_id and uc.business_id = p_business_id and uc.effective_from <= p_date
    order by uc.effective_from desc limit 1
  ) h on true
  where c.business_id = p_business_id and c.employee_id = p_employee_id and c.operational_date = p_date;

  return coalesce(v_result, jsonb_build_object('sale_cents', 0, 'cost_cents', 0, 'items', '[]'::jsonb));
end;
$$;

revoke all on function public.get_employee_day_consumption(bigint, bigint, date) from public, anon;
grant execute on function public.get_employee_day_consumption(bigint, bigint, date) to authenticated;
