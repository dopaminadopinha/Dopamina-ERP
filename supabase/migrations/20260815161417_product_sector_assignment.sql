-- Product sector is the durable classification chosen in the ERP. Zig syncs may
-- refresh product metadata, but must never replace this association.
create or replace function private.preserve_product_sector_on_service_sync()
returns trigger
language plpgsql
security invoker
set search_path = ''
as $$
begin
  if current_user = 'service_role' and new.item_type = 'product'::public.item_type then
...
drop trigger if exists preserve_product_sector_on_service_sync on public.items;
create trigger preserve_product_sector_on_service_sync
before insert or update of area_id, zig_product_id on public.items
for each row execute function private.preserve_product_sector_on_service_sync();
...
begin
  ...
  if p_area_id is not null and not exists (
    ...
    where a.id = p_area_id
      ...
      and upper(trim(a.name)) in ('BAR', 'CERVEJA', 'DRINKS', 'COZINHA', 'CHURRASQUEIRA')
      and a.is_active
  ) then
...
revoke all on function public.assign_products_to_sector(bigint, bigint[], bigint) from public, anon;
grant execute on function public.assign_products_to_sector(bigint, bigint[], bigint) to authenticated;

comment on function public.assign_products_to_sector(bigint, bigint[], bigint) is
  'Classifica até 500 produtos em um setor canônico do negócio e registra a alteração.';
    ...
    'products', (
      ...
      from (
        select i2.item_id, max(i2.product_name) as name, max(i2.product_sku) as sku,
          ...
          coalesce(max(case upper(trim(a.name))
            when 'BAR' then 'Bar'
...
begin
  ...
  with item_rows as (
    select
      case upper(trim(coalesce(a.name, '')))
        when 'BAR' then 'Bar'
        when 'CERVEJA' then 'Bar'
        when 'DRINKS' then 'Drinks'
        when 'COZINHA' then 'Cozinha'
...
comment on function public.get_sector_profitability(bigint, date, date) is
  'Rentabilidade por setor usando a classificação persistida no cadastro do produto.';
