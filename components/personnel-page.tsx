  ...
  return <section className="personnel-page">
    ...
    {loading ? <div className="personnel-loading">Carregando dados reais…</div> : <>
      {tab === "overview" && <>
        ...
        <article className="personnel-card"><Header kicker="Histórico" title="Últimos lançamentos"/><Obligations shifts={data.shifts.slice(0,5)} costs={data.costs.slice(0,5)} employees={data.employees} onPayment={togglePayment}/></article>
      </>}
...
function Obligations({shifts,costs,employees,onPayment}:{shifts:Shift[];costs:PersonnelCost[];employees:Employee[];onPayment:(type:"work_shift"|"personnel_cost",id:string,paid:boolean)=>Promise<void>}) {
  ...
  return <div className="obligation-list">{rows.length?rows.map(row=><div key={`${row.type}-${row.id}`}><span><b>{row.name}</b><small>{dateLabel(row.date)} · {row.description}{row.pix?` · Pix: ${row.pix}`:''}</small></span><strong>{row.amount>0?MONEY.format(row.amount):'Somente horas'}</strong><Status value={row.status}/>{row.amount>0&&<button onClick={()=>onPayment(row.type,row.id,row.status!=='paid')}>{row.status==='paid'?'Reabrir':'Marcar pago'}</button>}</div>):<Empty text="Nenhum lançamento no período selecionado."/>}</div>;
}
