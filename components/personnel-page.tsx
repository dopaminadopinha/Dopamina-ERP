...
export function PersonnelPage({ businessId, userId, range, onExpensesChanged }: { businessId: string; userId: string; range: Range; onExpensesChanged: () => Promise<void> }) {
  ...
  return <section className="personnel-page">
    ...
    {modal === "week" && <WeeklyShiftModal businessId={businessId} employees={data.employees.filter(e=>e.is_active)} range={range} onClose={()=>setModal(null)} onSaved={refreshed}/>}
...
function ShiftModal({businessId,employees,onClose,onSaved}:{businessId:string;employees:Employee[];onClose:()=>void;onSaved:()=>Promise<void>}) {
 ...
 const [form,setForm]=useState({employee_id:firstEmployee?.id||'',shift_date:isoToday(),start_time:'18:00',end_time:'02:00',break_minutes:'0',rate:firstEmployee?.default_hourly_rate?String(firstEmployee.default_hourly_rate):'',notes:''});const [busy,setBusy]=useState(false);const [message,setMessage]=useState('');const calculatedHours=timeHours(form.start_time,form.end_time,decimal(form.break_minutes));const rate=decimal(form.rate);const amount=calculatedHours*rate;
 function selectEmployee(employeeId:string){const employee=employees.find(row=>String(row.id)===employeeId);setForm({...form,employee_id:employeeId,rate:employee?.default_hourly_rate?String(employee.default_hourly_rate):form.rate});}
...
function datesInclusive(start:string,end:string){const startDate=new Date(`${start}T12:00:00Z`);const endDate=new Date(`${end}T12:00:00Z`);if(Number.isNaN(startDate.getTime())||Number.isNaN(endDate.getTime())||startDate>endDate)return[];const days=Math.round((endDate.getTime()-startDate.getTime())/86400000)+1;return Array.from({length:Math.min(days,31)},(_,index)=>new Date(startDate.getTime()+index*86400000).toISOString().slice(0,10));}
const WEEKDAY_LABEL=new Intl.DateTimeFormat('pt-BR',{weekday:'short',timeZone:'UTC'});

function WeeklyShiftModal({businessId,employees,range,onClose,onSaved}:{businessId:string;employees:Employee[];range:Range;onClose:()=>void;onSaved:()=>Promise<void>}) {
 const firstEmployee=employees[0];
 ...
 const [rate,setRate]=useState(firstEmployee?.default_hourly_rate?String(firstEmployee.default_hourly_rate):'');
 const [breakMinutes,setBreakMinutes]=useState('0');
 const [notes,setNotes]=useState('');
 const [days,setDays]=useState<Record<string,{checked:boolean;start_time:string;end_time:string}>>({});
 const [busy,setBusy]=useState(false);
 const [message,setMessage]=useState('');
 const dateList=useMemo(()=>datesInclusive(periodStart,periodEnd),[periodStart,periodEnd]);
 function selectEmployee(id:string){const employee=employees.find(row=>String(row.id)===id);setEmployeeId(id);setRate(employee?.default_hourly_rate?String(employee.default_hourly_rate):rate);}
 function toggleDay(date:string){setDays({...days,[date]:{checked:!(days[date]?.checked??false),start_time:days[date]?.start_time??'18:00',end_time:days[date]?.end_time??'02:00'}});}
 function updateDay(date:string,field:'start_time'|'end_time',value:string){setDays({...days,[date]:{...days[date],[field]:value}});}
 function applyFirstToAll(){const checkedDates=dateList.filter(date=>days[date]?.checked);if(!checkedDates.length)return;const first=days[checkedDates[0]];const next={...days};checkedDates.forEach(date=>{next[date]={...next[date],start_time:first.start_time,end_time:first.end_time};});setDays(next);}
 ...
 return <Modal title="Lançar semana" subtitle="Marque os dias trabalhados e o horário de cada um; tudo é lançado de uma vez." onClose={onClose}>
  <form onSubmit={save} className="personnel-form">
   <label><span>Funcionário</span><select value={employeeId} onChange={e=>selectEmployee(e.target.value)}><option value="">Selecione</option>{employees.map(row=><option key={row.id} value={row.id}>{row.name}</option>)}</select></label>
   <label><span>Valor da hora</span><input value={rate} onChange={e=>setRate(e.target.value)} inputMode="decimal" placeholder="0,00"/></label>
   <label><span>De</span><input type="date" value={periodStart} onChange={e=>setPeriodStart(e.target.value)}/></label>
   <label><span>Até</span><input type="date" value={periodEnd} onChange={e=>setPeriodEnd(e.target.value)}/></label>
   <label><span>Intervalo não pago por dia (min)</span><input value={breakMinutes} onChange={e=>setBreakMinutes(e.target.value)} inputMode="numeric"/></label>
   <div className="wide week-day-list">
    <div className="week-day-list-head"><span>Dias trabalhados</span>{checkedCount>1&&<button type="button" onClick={applyFirstToAll}>Repetir horário do 1º dia marcado</button>}</div>
    {dateList.length?dateList.map(date=>{
     const day=days[date]||{checked:false,start_time:'18:00',end_time:'02:00'};
     return <div key={date} className={`week-day-row ${day.checked?'checked':''}`}>
      <label className="check-label"><input type="checkbox" checked={day.checked} onChange={()=>toggleDay(date)}/><span>{WEEKDAY_LABEL.format(new Date(`${date}T12:00:00Z`))} {dateLabel(date)}</span></label>
      {day.checked&&<><input type="time" value={day.start_time} onChange={e=>updateDay(date,'start_time',e.target.value)}/><input type="time" value={day.end_time} onChange={e=>updateDay(date,'end_time',e.target.value)}/></>}
     </div>;
    }):<Empty text="Escolha um período válido."/>}
   </div>
   <label className="wide"><span>Observações</span><input value={notes} onChange={e=>setNotes(e.target.value)}/></label>
   {message&&<p className="form-message">{message}</p>}
   <Actions busy={busy} label={checkedCount?`Lançar ${checkedCount} dia(s)`:'Lançar'}/>
  </form>
 </Modal>;
}
