...
function SalesPage(props: Parameters<typeof SectionContent>[0]) {
  ...
  async function discoverOpenBills() { setDiscovering(true); setDiscoverMessage(""); const { data: session } = await supabase.auth.getSession(); const token = session.session?.access_token; if (!token) { setDiscoverMessage("Sua sessão expirou. Entre novamente."); setDiscovering(false); return; } try { const response = await fetch("/api/zig/discover-open-bills", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ date: props.range.end }) }); const result = await response.json() as { error?: string; results?: { path: string; ok: boolean; status: number | null; error: string | null }[] }; if (!response.ok) throw new Error(result.error ?? "Falha no diagnóstico."); const found = result.results?.filter((row) => row.ok) ?? []; setDiscoverMessage(found.length ? `Endpoints que responderam: ${found.map((row) => row.path).join(", ")}. Peça pro Claude conferir os detalhes.` : "Nenhum dos endpoints testados respondeu. Peça pro Claude conferir os detalhes."); } catch (error) { setDiscoverMessage(error instanceof Error ? error.message : "Não foi possível testar os endpoints."); } finally { setDiscovering(false); } }
  return <section className="sales-page"><ModuleHero eyebrow="Resultados" title="Vendas e faturamento" description={apiHasData ? "Visão gerencial das vendas reais sincronizadas da Zig; estornos são excluídos." : "Sem dados da API neste período; exibindo o relatório disponível quando houver."} action={syncing ? "Sincronizando..." : "Sincronizar agora"} icon={<ShoppingCart size={19} />} onAction={syncing ? undefined : synchronize} />
    ...
    <div className="sync-message"><button type="button" onClick={discoverOpenBills} disabled={discovering} style={{cursor:"pointer"}}>{discovering ? "Testando…" : "Diagnóstico temporário: testar contas em aberto da Zig"}</button>{discoverMessage ? ` — ${discoverMessage}` : ""}</div>
