...
function DrePage(props: Parameters<typeof SectionContent>[0]) {
  ...
  return <section className="dre-page">
    ...
    {current.cmvMissingRevenue > 0 && <DismissibleNotice noticeKey="dre-cmv-incompleto"><TriangleAlert size={14} /><span><strong>{MONEY.format(current.cmvMissingRevenue)}</strong> da receita ainda não tem CMV confiável ({NUMBER.format(current.cmvCoverage * 100)}% de cobertura) — o resultado acima é o resultado conhecido, não uma estimativa completa.</span></DismissibleNotice>}
    ...
    <div className="dre-layout">
      ...
      <article className="chart-card"><div className="card-title-row"><div><p>Detalhamento</p><h3>Despesas operacionais por categoria</h3></div></div><div className="expense-category-scroll"><ExpenseCategoryBars rows={current.opexCategories} total={current.opex} /></div></article>
    </div>
...
function CmvPage(props: Parameters<typeof SectionContent>[0]) {
  ...
  return <section><ModuleHero eyebrow="Rentabilidade" title="CMV e margem de lucro" description={automaticRows.length ? "Calculado automaticamente sobre as vendas sincronizadas e os custos do cadastro central." : "Exibindo o relatório de CMV disponível; sincronize vendas para automatizar o cálculo."} action="Revisar custos" icon={<CircleDollarSign size={19} />} onAction={() => props.setSection("cadastros")} />
    ...
    {missingCount > 0 && <DismissibleNotice noticeKey="cmv-produtos-sem-custo"><TriangleAlert size={14} /><span><strong>{missingCount} produto(s)</strong> sem custo confiável · {MONEY.format(missingRevenue)} da receita fora do CMV conhecido</span></DismissibleNotice>}
...
function SectorProfitabilityPage(props: Parameters<typeof SectionContent>[0]) {
  ...
  return <section className="sector-page"><ModuleHero eyebrow="Gestão por container" title="Rentabilidade por setor" description="Compare quanto cada container vende, quanto tem de custo conhecido e quanto resultado entrega." action="Revisar custos" icon={<CircleDollarSign size={19} />} onAction={() => props.setSection("cadastros")} />
    ...
    {!operationalAreas.length && <DismissibleNotice noticeKey="setores-nenhum-operacional"><TriangleAlert size={19} /><div><strong>Nenhum setor operacional cadastrado</strong><span>Marque ao menos um setor como operacional em Cadastros → Setores para ver a rentabilidade por setor.</span></div></DismissibleNotice>}
    {selected && <section className="sector-detail"><div className="sector-detail-head"><div><p>Detalhamento do container</p><h3>{selected.name}</h3><span>{selected.products.length} produto(s) · cobertura de CMV em {NUMBER.format(selected.coverage * 100)}% do faturamento</span></div><div className="sector-tabs" role="tablist" aria-label="Selecionar setor">{operationalAreas.map((area) => <button type="button" role="tab" aria-selected={selectedSector === area.name} className={selectedSector === area.name ? "active" : ""} key={area.id} onClick={() => setSelectedSector(area.name)}>{area.name}</button>)}</div></div>
      ...
      {selected.missingRevenue > 0 && <DismissibleNotice noticeKey={`setor-cmv-incompleto-${selected.name}`}><TriangleAlert size={14} /><span><strong>{MONEY.format(selected.missingRevenue)}</strong> do faturamento deste setor ainda não tem CMV confiável e não entra no lucro conhecido.</span></DismissibleNotice>}
...
function StockPage(props: Parameters<typeof SectionContent>[0]) {
  ...
  stock.movements.filter((movement) => Number(movement.quantity) < 0).forEach((movement) => consumedByItem.set(String(movement.item_id), (consumedByItem.get(String(movement.item_id)) ?? 0) + Math.abs(Number(movement.quantity))));
  return <section className="stock-page"><ModuleHero eyebrow="Operação e controle" title="Estoque virtual e inventário" description="Acompanhe o saldo teórico, conte o estoque físico e preserve cada diferença no histórico." action="Nova contagem" icon={<Boxes size={19} />} onAction={() => setInventoryOpen(true)} />
    ...
    {stockError && <DismissibleNotice noticeKey="estoque-erro-carregamento"><TriangleAlert size={18} /><div><strong>{stockError}</strong><span>Nenhum saldo foi inventado ou alterado.</span></div><button type="button" onClick={reloadStock}>Tentar novamente</button></DismissibleNotice>}
...
function calculateConsumableUsage(items: StockItem[], countRows: InventoryCountUsageRow[], receiptRows: PurchaseReceiptUsageRow[]): ConsumableUsage[] {
  ...
  return items.map((item) => {
    ...
    return {
      itemId: String(item.id), name: item.name, itemType: item.item_type, unit: item.unit, countCount: counts.length, intervalCount, invalidIntervals,
...
function ConsumableUsageList({ rows, query, onQueryChange, loading, error, onInventory }: { rows: ConsumableUsage[]; query: string; onQueryChange: (value: string) => void; loading: boolean; error: string; onInventory: () => void }) {
  ...
  const filtered = rows.filter((row) => row.name.toLocaleLowerCase("pt-BR").includes(normalizedQuery) && (itemType === "all" || row.itemType === itemType));
  const insufficient = filtered.filter((row) => row.status === "insufficient").length;
  const typeLabel: Record<StockItem["item_type"], string> = { product: "Produto vendido", consumable: "Consumível", ingredient: "Ingrediente" };
  return <article className="stock-history-section consumable-usage-section">
    <div className="stock-section-heading"><div><p>Consumo por contagem</p><h3>Consumo médio dos produtos</h3><span>A média usa contagens físicas consecutivas e soma somente compras já recebidas entre elas.</span></div><button type="button" onClick={onInventory}><Plus size={15} /> Nova contagem</button></div>
    <div className="module-toolbar consumable-usage-toolbar"><label><Search size={16} /><input value={query} onChange={(event) => onQueryChange(event.target.value)} placeholder="Buscar produto no estoque" /></label><select value={itemType} onChange={(event) => setItemType(event.target.value)} aria-label="Filtrar consumo médio por tipo"><option value="all">Todos os tipos</option><option value="product">Produtos vendidos</option><option value="consumable">Consumíveis</option><option value="ingredient">Ingredientes</option></select><span className="table-count">{filtered.length} item(ns) · {insufficient} sem dados suficientes</span></div>
    {loading ? <p className="stock-loading"><Clock3 size={15} />Calculando os intervalos entre contagens...</p> : error ? <DismissibleNotice noticeKey="consumo-medio-erro"><TriangleAlert size={18} /><div><strong>{error}</strong><span>Nenhuma média foi estimada.</span></div></DismissibleNotice> : <div className="data-table-card consumable-usage-scroll"><div className="consumable-usage-table"><div className="consumable-usage-row header"><span>Produto</span><span>Período medido</span><span>Consumo medido</span><span>Média / dia</span><span>Média / semana</span><span>Média / mês</span><span>Situação</span></div>{filtered.length ? filtered.map((row) => <div className="consumable-usage-row" key={row.itemId}><strong>{row.name}<small>{typeLabel[row.itemType]} · {row.unit} · {row.countCount} contagem(ns)</small></strong><span>{row.firstCountedAt && row.lastCountedAt && row.countCount >= 2 ? `${new Date(row.firstCountedAt).toLocaleDateString("pt-BR")} a ${new Date(row.lastCountedAt).toLocaleDateString("pt-BR")}` : "—"}<small>{row.totalDays ? `${row.totalDays} dia(s) · ${row.intervalCount} intervalo(s)` : ""}</small></span><span>{row.totalConsumption === null ? "—" : `${NUMBER.format(row.totalConsumption)} ${row.unit}`}</span><strong>{row.dailyAverage === null ? "—" : `${NUMBER.format(row.dailyAverage)} ${row.unit}`}</strong><strong>{row.weeklyAverage === null ? "—" : `${NUMBER.format(row.weeklyAverage)} ${row.unit}`}</strong><strong>{row.monthlyAverage === null ? "—" : `${NUMBER.format(row.monthlyAverage)} ${row.unit}`}</strong><span>{row.status === "insufficient" ? <small className="usage-status insufficient">Dados insuficientes</small> : row.status === "review" ? <small className="usage-status review">Revisar contagens</small> : <small className="usage-status known">Calculado</small>}{row.status === "insufficient" && <small className="usage-status-note">Faça pelo menos 2 contagens físicas.</small>}{row.status === "review" && <small className="usage-status-note">{row.invalidIntervals} intervalo(s) inconsistente(s) ignorado(s).</small>}</span></div>) : <div className="consumable-usage-empty"><PackageSearch size={22} /><strong>Nenhum item encontrado</strong><span>Ajuste a busca ou o filtro de tipo.</span></div>}</div></div>}
...
function groupedStockMovements(rows: StockMovement[]): GroupedStockMovement[] {
  const groups = new Map<string, { row: GroupedStockMovement; costTotal: number; costQuantity: number; origins: Set<string> }>();
  rows.forEach((movement) => {
    const day = isoDateInSaoPaulo(movement.occurred_at);
    const key = `${day}|${movement.item_id}|${movement.movement_type}|${movement.unit}|${movement.sector}`;
    const quantity = Number(movement.quantity);
    const cost = movement.unit_cost === null ? null : Number(movement.unit_cost);
    const current = groups.get(key);
    if (!current) {
      groups.set(key, {
        row: { ...movement, day, quantity, entryCount: 1 },
        costTotal: cost === null ? 0 : Math.abs(quantity) * cost,
        costQuantity: cost === null ? 0 : Math.abs(quantity),
        origins: new Set([movement.origin]),
      });
      return;
    }
    current.row.quantity = Number(current.row.quantity) + quantity;
    current.row.entryCount += 1;
    current.origins.add(movement.origin);
    if (cost !== null) { current.costTotal += Math.abs(quantity) * cost; current.costQuantity += Math.abs(quantity); }
  });
  return [...groups.values()].map((group) => ({
    ...group.row,
    unit_cost: group.costQuantity > 0 ? group.costTotal / group.costQuantity : null,
    origin: group.origins.size === 1 ? [...group.origins][0] : `${group.row.entryCount} registros agrupados`,
  })).sort((a, b) => b.day.localeCompare(a.day) || a.name.localeCompare(b.name, "pt-BR"));
}
...
function CatalogPage(props: Parameters<typeof SectionContent>[0]) {
  ...
  const [editingIngredient, setEditingIngredient] = useState<CatalogItem | "new-ingredient" | "new-consumable" | null>(null);
  const [query, setQuery] = useState("");
  const [view, setView] = usePersistedTab<"products" | "ingredients" | "consumables" | "areas">("dopamina:catalog-tab", "products", CATALOG_TABS);
  ...
  const ingredients = props.data.ingredients.filter((item) => item.item_type === "ingredient");
  const consumables = props.data.ingredients.filter((item) => item.item_type === "consumable");
  const source = view === "products" ? props.data.catalogItems : view === "consumables" ? consumables : ingredients;
  const rows = source.filter((item) => {
    ...
    const matchesSector = view !== "products" || sectorFilter === "all" || (sectorFilter === "unassigned" ? sector === null : sector === sectorFilter);
    return matchesQuery && matchesSector && (view !== "products" || statusFilter === "all" || statusOf(item) === statusFilter);
  });
  ...
  const creatingConsumable = view === "consumables";
  return <section><ModuleHero eyebrow="Base central" title="Produtos e fichas técnicas" description="Classifique os produtos reais da Zig por setor e mantenha custos, consumíveis e fichas técnicas no mesmo cadastro." action={creatingConsumable ? "Novo consumível" : "Novo ingrediente"} icon={<ClipboardList size={22} />} onAction={() => setEditingIngredient(creatingConsumable ? "new-consumable" : "new-ingredient")} />
    ...
    {editingIngredient && <IngredientModal businessId={props.businessId} item={typeof editingIngredient === "string" ? null : editingIngredient} itemType={editingIngredient === "new-consumable" || (typeof editingIngredient !== "string" && editingIngredient.item_type === "consumable") ? "consumable" : "ingredient"} onClose={() => setEditingIngredient(null)} onSaved={async () => { await props.onRefresh(); setEditingIngredient(null); }} />}
  </section>;
}
...
function IngredientModal({ businessId, item, itemType, onClose, onSaved }: { businessId: string; item: CatalogItem | null; itemType: "ingredient" | "consumable"; onClose: () => void; onSaved: () => Promise<void> }) {
  useEscapeToClose(onClose);
  ...
  async function save(event: React.FormEvent) {
    ...
    if (item) {
      ...
      const categoryResult = itemType === "consumable" ? await supabase.from("categories").select("id").eq("business_id", Number(businessId)).eq("kind", "consumable").order("id").limit(1).maybeSingle() : null;
      const created = await supabase.from("items").insert({ business_id: Number(businessId), category_id: categoryResult?.data?.id ?? null, name: name.trim(), item_type: itemType, consumption_unit: unit.trim(), average_unit_cost: parsedCost, latest_unit_cost: parsedCost }).select("id").single();
      if (created.error || !created.data) { setBusy(false); return setMessage(`Não foi possível criar o ${itemType === "consumable" ? "consumível" : "ingrediente"}. Verifique se o nome já existe.`); }
  ...
  const typeLabel = itemType === "consumable" ? "consumível" : "ingrediente";
  return <div className="modal-backdrop"><form className="modal-card ingredient-modal" role="dialog" aria-modal="true" aria-labelledby="ingredient-editor-title" onSubmit={save}><button type="button" className="modal-close" onClick={onClose} aria-label="Fechar"><X size={19} /></button><p className="page-kicker">{itemType === "consumable" ? "Consumível" : "Ingrediente"}</p><h2 id="ingredient-editor-title">{item ? `Editar ${item.name}` : `Novo ${typeLabel}`}</h2><p className="modal-description">{itemType === "consumable" ? "Cadastre itens sem venda direta, como papel higiênico, guardanapos e produtos de limpeza." : "Cadastre o custo na unidade usada na receita. Exemplo: vodka em ml e limão por unidade."}</p><div className="form-grid"><label className="span-2"><span>Nome</span><input value={name} onChange={(event) => setName(event.target.value)} placeholder={itemType === "consumable" ? "Ex.: Papel higiênico" : "Ex.: Vodka"} autoFocus /></label><label><span>Unidade de controle</span><select value={unit} onChange={(event) => setUnit(event.target.value)}><option value="un">unidade</option><option value="rolo">rolo</option><option value="pacote">pacote</option><option value="caixa">caixa</option><option value="ml">ml</option><option value="g">g</option><option value="l">litro</option><option value="kg">kg</option></select></label><label><span>Custo por {unit}</span><input value={cost} onChange={(event) => setCost(event.target.value)} inputMode="decimal" placeholder="0,00" /></label><label className="span-2"><span>Aplicar a partir de</span><input type="date" value={effectiveFrom} onChange={(event) => setEffectiveFrom(event.target.value)} /></label></div>{message && <p className="modal-message">{message}</p>}<div className="modal-actions"><button type="button" className="modal-secondary" onClick={onClose}>Cancelar</button><button className="modal-primary" disabled={busy}>{busy ? "Salvando..." : `Salvar ${typeLabel}`}</button></div></form></div>;
}
...
function ExpensesPage(props: Parameters<typeof SectionContent>[0]) {
  ...
  return <section><ModuleHero eyebrow="Financeiro" title="Despesas" description="Entenda onde o dinheiro é gasto e quanto custa manter o Dopamina funcionando." action="Nova despesa" icon={<ReceiptText size={19} />} onAction={() => setEditing("new")} />
    ...
    <div className="data-table-card expenses-table-scroll"><div className="responsive-table expenses-table"><div className="table-row table-header"><span>Data</span><span>Descrição</span><span>Categoria</span><span>Tipo</span><span>Recorrência</span><span>Status</span><span className="expense-col-value">Valor</span><span className="expense-col-actions-header">Ações</span></div>{rows.length ? rows.map((expense) => {
      ...
      return <div className="table-row" key={expense.id}>
        <span className="expense-col-date">{dateLabel(expense.expense_date)}</span>
        <strong className="expense-col-description" title={expense.description}>{expense.description}<small className="expense-payment-hint">{expense.purchase_id ? "Gerado e controlado pela compra" : expense.source_type ? "Gerado e controlado pelo módulo de origem" : expense.payment_method || "Pagamento não informado"}</small></strong>
        <span className="expense-col-category" title={expense.category}>{expense.category}</span>
        <span><span className={`expense-type-badge ${expense.cost_behavior}`}>{expense.cost_behavior === "fixed" ? "Fixa" : "Variável"}</span></span>
        <span>{expense.is_recurring ? <small className="recurring-badge">Recorrente</small> : <span className="expense-single-label">Não recorrente</span>}</span>
        <span><StatusBadge status={expense.status} /></span>
        <strong className="expense-col-value">{MONEY.format(expense.amount)}</strong>
        <span className="row-actions expense-col-actions">{canTogglePayment && <button className={`expense-pay-toggle ${expense.status === "completed" ? "is-paid" : ""}`} onClick={() => togglePayment(expense)}>{expense.status === "completed" ? "Reabrir" : "Marcar pago"}</button>}<ExpenseRowMenu label={expense.description} onEdit={canEdit ? edit : undefined} onDelete={canDelete ? del : undefined} /></span>
      </div>;
